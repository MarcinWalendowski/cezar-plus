...
| D27 | Does an approved note proposal stop at the spec, and what bounds it if it does not? | **It continues automatically into implementation, bounded by a step budget per run — and by nothing else.** Owner decision, 2026-08-15, recorded in `cezar/.ai/specs/2026-08-15-autonomous-implementation-continuation.md`. A concurrency cap, halt-on-first-failure, and never-auto-push were each offered as additional bounds and **explicitly declined**. Three things this binds, for every implementer: **(a)** a budget-exhausted run lands in **`review` with `stopReason: 'budget'`** — never `done`, and never `failed`. **(b)** `RunStatus` is **not** widened; `stopReason` is a new optional field. **(c)** the built-in implementation workflow commits locally and **does not push**. | (a) is the whole safety property. An agent halted mid-edit and an agent that completed both end with "the run is over", so a shared terminal state makes a note that half-built a feature in four repos present as four successes, discovered by reading the code weeks later. `failed` is equally wrong in the other direction: an agent that errored and an agent we stopped are different facts, and collapsing them makes "bigger budget or bug fix?" unanswerable from the record. (b) cezar is a released npm package where backward compatibility wins, and adding a member to a published union breaks consumers switching exhaustively over it; `review` already means "stopped, a human must look", which is exactly true here. (c) declining never-auto-push as a *bound* is a decision not to add a constraint, **not** an instruction to push — pushing is outward-facing and irreversible in a way a local commit is not, and the standing push authorization covers a person exercising judgement in a session, not an agent triggered by a note typed on a phone. One line to flip when the owner wants it. **Name the escalation rather than inheriting it:** `NOTE_TO_SPEC_WORKFLOW` withholds a general shell on purpose, its docblock saying such an agent "can install dependencies, run migrations and push branches while nobody is watching, on the strength of a note somebody typed on their phone" — an implementing agent needs that shell for the repo's gates, so autonomy is a real privilege escalation over what ships today and is recorded here as a knowing trade. **AMENDED 2026-08-15 (twice).** *(i)* **A budget unit is one check-step attempt OR one agent turn, NOT one entry into `execute()`'s step loop.** The first implementation counted loop entries — the reading "step budget" invites — and was close to decorative for the shape this feature uses: `execute()` iterates `workflow.steps`, a **fixed-length** array, so a single-step workflow (`QUICK_TASK_WORKFLOW` today, `AUTONOMOUS_IMPLEMENTATION_WORKFLOW` tomorrow) enters it once and the counter stops at 1, while the agent inside takes unbounded further turns through `sendMessage` / `CEZ:MONITORING` / monitoring wake-ups, all reusing the same open session via `turn-end`, which the loop never re-enters. Counting is now a persisted `RunRecord.stepsUsed` spent at the loop top **and** both `turn-end` handlers. The generalisable lesson: the enforcement code was correct as written and counted the wrong thing, so reading a diff confirms a counter is right, and only asking "what can happen without passing this line?" confirms it counts work. *(ii)* `config.stepBudget` defaults to **0 = unlimited**, which is right for Phase 1 (a nonzero repo-wide ceiling would have changed every existing manual and retry-heavy run the moment it shipped) and is a **trap for Phase 3**: an autonomous run started under that default is unbounded, and the step budget is the *only* bound (a) names. Phase 3 must therefore refuse to start an autonomous continuation with no budget, or carry its own non-zero default. **"Autonomous and unbounded" must be unreachable**, and a guard asserting so is Phase 3 verification. |
