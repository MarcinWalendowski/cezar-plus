...
|---|----------|-----------------|-----|----------|
| Q1 | Which parked sessions become durable? | Only turns ending in `CEZ:MONITORING`; ordinary `waiting` sessions keep the 15-minute timeout. **(Partially superseded 2026-08-20 by `2026-08-20-inactive-sessions-stay-in-progress.md`: ordinary `waiting`/`ASK` still keep the 15-minute timer — the memory bound is intact — but its firing now PARKS the run at `waiting` (in-progress / needs-you) and resumes via `--resume`, instead of expiring it to `done`. "expire as today" below is no longer true: an unanswered handoff is never recorded as finished.)** | The marker is an explicit agent-owned liveness declaration, while an unanswered user handoff can safely expire as today. | ok |
