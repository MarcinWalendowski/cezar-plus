...
| R3 | **SUPERSEDED 2026-08-29 by `.ai/specs/2026-08-29-per-retry-step-timing.md`.** `iterations > 1`: `startedAt` is **overwritten** on each attempt (`run.ts:3360`), so the rail shows the *current attempt*, not the cumulative cost. | ~~Accepted and stated: the row already carries the `×N` badge (`step-rail.tsx:93-97`); the clock's `title` says "this attempt". Cumulative-across-attempts would need a new persisted field — out of the web-only class, and not asked for.~~ It has since been asked for. `StepState` now carries an additive `attempts` array (`{ n, startedAt, endedAt? }` per attempt), maintained by `RunStore.updateStep` off the status transition, and the rail renders a per-attempt tree plus an aggregate clock once a step's array is complete and holds more than one entry. A step with no covering `attempts` array (every record written before that spec shipped) still renders exactly as described here. |
