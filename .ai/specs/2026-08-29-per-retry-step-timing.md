...
**RECONCILED 2026-08-29 — that hand-diff was done, and the branch is now merged.** The union was
taken deliberately, not mechanically:

- **Dropped, all of it:** this branch's engine, store, contract and UI changes, plus the three
  engine tests that pin them (`recover-attempt-timing.test.ts` and the additions to
  `broker-retry.test.ts` / `recover-session-failure.test.ts`). `main`'s design is strictly better
  on the one point this branch's `run.ts` surgery existed to fix — closing an attempt at the
  moment of failure rather than at the next open. `main` solves it **once, in the store**
  (`accumulateStepAttempts` rule 2: any step leaving the active set closes its open attempt,
  which covers "six of the nine retry paths"), where this branch edited each retry path in the
  engine individually. `store.test.ts` already covers the case, including *"pending with no
  re-entry closes the attempt at the injected clock"*.
- **Kept:** the real-browser E2E and its fixture — the one thing `main`'s version lacked — plus
  the `record` (video) operation added to `.ai/browsers/agent-browser.md` and
  `packages/web/e2e/agent-browser.ts`, which is feature-independent, and this spec as the record.
- **Re-pointed, not copied:** the fixture was rewritten field by field (`endedAt` →
  `finishedAt`, `n` dropped) and the E2E's traversal rewritten, because `main` renders the
  breakdown behind the `×N` badge as a `step-attempts` sibling rather than as always-visible
  sibling rows. `fixtures/README.md` now says plainly that the record is no longer a pure
  capture, so it can no longer be cited as proof that `RunStore.updateStep` writes that shape.
- **Also dropped:** this branch's `BACKWARD_COMPATIBILITY.md` line, which documented
  `{ n, startedAt, endedAt? }` — a shape that does not exist on `main`. The correct entry for the
  landed `{ startedAt, finishedAt? }` was already there.

The R3 correction in `.ai/specs/2026-08-20-step-and-tool-call-durations.md` conflicted on both
sides; `main`'s was kept because it names the spec that actually shipped. This branch's two
corrections in that file — that *"this box has no browser"* is false — were kept, being true
regardless of which timing design landed.
