...
**Result 2026-08-30: all five exit 0** (typecheck, `npm test` 12,397 passed / 0 failed across 659
files, `test:unit`, `build`, `test:package` 45/45). Getting there needed three repairs, recorded
here because two of them were not this feature's own work:

1. **A merge regression, not a feature bug.** `b7073cfe` restored the feature-side
   `packages/web/src/api/queries.ts` wholesale over the `e67bd9cf` merge and dropped
   `useRunnerLock`, which `origin/main` had added for `2026-08-29-global-provider-toggle.md` (D9).
   One missing export broke **503 of 510** failing tests across 18 files, nearly all in cockpit
   suites this feature never touched. Restored with its `LockableRunner` import; every changed
   package was then re-diffed against `origin/main` to confirm nothing else was dropped the same
   way. When resolving a conflict by taking one side of a file wholesale, diff that file against
   the base afterwards.
2. **An O(rows^2) in `SourceStore`, a real production cost.** `load()` runs on every store-lock
   acquisition and re-read plus Zod-validated the entire log and comments files only to recover
   the last `seq`, so every `appendLog` re-validated up to the 10,000 rows `maybeCompact`
   deliberately allows. Replaced with a bounded tail read (`store.ts` `lastNdjsonSeq`); the suite
   went from 87s to 1.6s. Two regression tests cover the subtle cases (a torn trailing line must
   not reset the sequence and reissue live ids; a row larger than the tail window must widen it),
   both mutation-proven by disabling the widening.
3. **A pre-existing failure unrelated to sources.** `test/e2e/package-cli.test.ts` still asserted
   the pre-rebrand `/cezar — local cockpit/` banner and fails identically on `origin/main`, since
   the `cezar-plus` rebrand changed the banner without updating this one assertion.

Six test-side defects in the feature's own suites were also fixed: two `sources-api.test.ts`
fixtures (a hardcoded `docId` that `computeDocId` correctly rejects, and an unknown-document
fixture asserting the "not in conflict" refusal), one self-contradictory `conflicts.test.ts`
assertion (`sink.read` recomputes `localVersion` from the bytes, so keep-local cannot change it;
the real property is that the *stored* stale hash gets refreshed), and three
`sources-section.test.tsx` fixtures (a missing `providers` stub, two clicks racing an async list).

Sections 2 and 4 (mutation and negative controls, runtime E2E) remain **unrun**, so this stays
QA Needed rather than Done.
