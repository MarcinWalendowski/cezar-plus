...
**CORRECTED 2026-08-24 (see `.ai/specs/2026-08-24-manual-deploy-not-a-bug.md`):** the *deploy* half of the standing authorization below, and the closing claim that a task on cezar ships end to end by default, are superseded for agent-run deploys. Since commit `c328ec06`, both targets in `.ai/deploy-targets.json` are `"manual": true`, so an agent-run task on cezar commits and pushes end to end and then **parks** at the `deploy` step: a person activates the service and resolves the handoff. That parked state is the expected terminal state for such a run, not a defect. The commit and `git push origin main` authorization below is current and unchanged; only the deploy claim is stale. Original text, unchanged, below:
