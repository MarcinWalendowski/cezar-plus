...
- **Gates first, fail closed.** Never commit/push/deploy a red build. Typecheck + lint + tests green is the precondition for the commit-push step, and the deploy step must gate on a real readiness probe and never ship a broken build.
- **Do NOT self-deploy from inside a running cockpit session — until the non-disruptive path lands.** `cezar server-deploy` today is a hard `systemctl restart cezar.service` with `KillMode=control-group`: it SIGKILLs every in-flight run **and the on-box deployer itself** (see `.ai/specs/2026-08-19-non-disruptive-cezar-self-deploy.md`, still draft). A backend change deployed from within a cockpit task kills that task mid-cutover before it can report. So run the deploy **detached** (`systemd-run` / outside `cezar.service`'s cgroup) or after the session ends. A **web-only** change swaps into `/opt/cezar` without a restart and is safe to ship live. Making self-deploy truly non-disruptive (Phase 1 of that spec: detach run processes from the server cgroup) is the prerequisite for "always deploy" being safe to automate — do that before wiring any unattended auto-deploy.
