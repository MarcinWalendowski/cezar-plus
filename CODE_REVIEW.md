...
- Spawned processes use `execFile`/`spawn` with argument arrays — never string-interpolated shell commands. **`allowedTools` is not a restriction, on any backend.** It reads like a per-step allowlist and is not one: measured against `claude` 2.1.224, `--allowedTools` only *grants* additively — `--permission-mode default --allowedTools Read` still ran `Bash`, and only `--disallowedTools` (which cezar-plus never emits) removes a tool from the surface. Claude runs pass `--permission-mode bypassPermissions` (2026-08-15, spec `.ai/specs/2026-08-15-bypass-permissions-claude-sessions.md`), so nothing prompts and nothing is denied; Codex and OpenCode never honored `allowedTools` either (Codex: its own sandbox, approvals off, network on; OpenCode: everything auto-approved) (#430). **Treat every run as having full shell access in its worktree and its `--add-dir` paths** — that was already this list's conclusion and it is the only part of the old wording that survived. The containment is the worktree boundary, not the tool list. *(Corrected 2026-08-15: this bullet previously claimed unapproved tools "are denied without prompting" under `--permission-mode dontAsk`, with `CEZ_APPROVAL_GATE=1` opting into an approval UI. The mode changed, the env var is deleted, and the denial half was never true to begin with.)*
