...
| `CEZ_ANALYTICS=0` | Turn **off** the workspace analytics sink (`.ai/specs/2026-08-29-filed-task-detail-page.md`). On by default: opening a filed task's detail page writes one `todo.detail_opened` line to `<CEZ_HOME>/analytics/events.ndjson`, a plain file on this machine's own disk that leaves it never — no account, no cloud, nothing sent anywhere. Only the exact value `0` disables it (`false`, `off` and an empty `CEZ_ANALYTICS=` all leave it on); with it off, `POST /workspace/analytics/events` still answers `202` and the sink drops the line, so a client never has to care whether the operator kept the log. |
...
| `CEZ_SWEEP_DELAY_MS` | Delay orphan-worktree reclamation after boot/context recovery. Defaults to `300000` (5 minutes); `0` schedules it immediately. |
| `CEZ_LEASE_STALE_MS` | Age after which a worktree lease no longer blocks reclamation. Defaults to `900000` (15 minutes). |
