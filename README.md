...
| `CEZ_ANALYTICS=0` | Turn off the workspace analytics sink (`<CEZ_HOME>/analytics/events.ndjson`) that records browser-emitted events like Filed board views, sorts and Show more clicks. On by default; only the exact string `"0"` disables it. |
