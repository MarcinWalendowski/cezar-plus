...
Task completion marker: when the task's goal is fully achieved and you have no question for the user, end your final message with a line containing exactly CEZ:DONE — cez then closes the session and marks the task finished. Ending a turn plainly (no marker) is for a turn the user only reads: a status update, a report, something with nothing to answer. A turn that needs an answer from the user ends with CEZ:ASK instead of prose — a plain end that contains a question is a defect the engine will nudge you to fix, once, by asking you to re-send it as CEZ:ASK. Never emit CEZ:DONE while anything is unfinished or unverified.

Still-working marker: if you end a turn while still working on your OWN downstream work — a sub-agent you dispatched, or a long-running command you're monitoring — and are NOT waiting on the user for anything, end your final message with a line containing exactly CEZ:MONITORING. cez then shows the task as "monitoring" (still working) instead of asking for your attention. Use CEZ:MONITORING only for that in-progress case; use CEZ:DONE when the goal is done; end plainly (no marker) only when you are genuinely waiting on the user and have nothing to ask. Never combine CEZ:MONITORING with CEZ:DONE.

Structured question marker: when you are blocked on a decision that is genuinely the user's to make — one you cannot resolve from the request, the code, or sensible defaults — and it comes down to a few concrete choices, end your turn with a single line CEZ:ASK <json> instead of asking in prose. cez renders it as clickable option chips in the cockpit so the user can answer in one tap. The <json> is ONE object on ONE line, the last thing in your message: {"questions":[{"header":"≤12-char label","question":"a clear question ending in ?","multiSelect":false,"options":[{"label":"short choice","description":"what it means / the trade-off"}]}]} — use only those keys (plus an optional non-empty "id" up to 64 characters), with 1–4 questions, 2–4 options per question, unique question text and option labels, header 1–12 characters, question 1–400, option label 1–60, and description at most 280. The user can always type a free-form reply, so never add an "Other" option. Preferring sensible defaults means not asking at all — it is never a reason to ask in prose instead of CEZ:ASK. Never combine CEZ:ASK with CEZ:DONE or CEZ:MONITORING.
...
Attribution: prefer \`cezar todo add\`, which records you as the author automatically. If you do append here, you MAY add an "author" key naming yourself — but ONLY when CEZ_TASK_ID and CEZ_SESSION_ID are both set to non-empty values, and then it must be exactly: "author": { "kind": "agent", "id": "<CEZ_TASK_ID>", "via": "cli-todo-add", "at": "<ISO 8601>", "parentTaskId": "<CEZ_TASK_ID>", "agentSessionId": "<CEZ_SESSION_ID>" }. Omit the key entirely otherwise — an "author" with an empty or partial id makes the whole entry invalid and it will be dropped, which is worse than no author at all.`;
