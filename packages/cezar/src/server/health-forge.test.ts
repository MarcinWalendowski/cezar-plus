...
describe('GET /api/v1/health — forge + capabilities', () => {
  ...
  it('local mode: keeps every pre-forge field and adds localHandoff:true + forge:null outside a repo', async () => {
    ...
    expect(body.capabilities).toEqual({
      ...
      sources: false,
      notes: false,
      // ON by default since 2026-08-16 — see `server/capabilities.ts`. Every assertion of this
      // payload in this file runs with `singleProject: false`, which is the topology the flip is
      // about; a single-project cockpit still reports it false regardless of the flag.
      workspaceViews: true,
      notify: false,
      accountUsage: false,
      skills: true,
  ...
  it('reports followups:true with CEZ_FOLLOWUPS=1', async () => {
    ...
    expect((await health()).capabilities).toEqual({
      ...
      knowledge: false,
      sources: false,
      // `true` since 2026-08-15: notes joined skills as an OPT-OUT capability (see
      // capabilities.test.ts — "notes (D1, inverted like skills)").
      notes: false,
      workspaceViews: true,
      notify: false,
      accountUsage: false,
