 ...
 *
 * Opt-out gate: skills from a configured team skills repo — see `gatedSkillsRepos` — appear
 * unless the user has curated them away. cezar ships no team repo by default, so this tier is
 * empty until `skillsRepos` names one. `importedSkills` in the GLOBAL `~/.cezar/ui-state.json` (not the
 ...
 * every configured team skill shows (the historical behavior — no upgrade break for existing
 * installs); a PRESENT array (even `[]`) means the user has taken control and only those
 * names show. This is the single chokepoint, so the decision is identical for every consumer —
 * catalog, composer picker, planner, runner.
 */
 ...
 * `undefined` means the key is absent — "not curated", so every configured team skill shows
...
/**
 * The opt-out gate: keep every team skill whose repo is NOT gated (a repo outside the
 * configured `skillsRepos` — auto-loads everything). For skills from a gated team repo,
 * `importedSkills === undefined` keeps them ALL (not curated — the historical behavior),
 * while a present array keeps only the named ones. Local skills carry no `team` and are
 * always kept. Pure so the gate is unit-testable without a network clone (the gated set
 * is a const default otherwise).
 */
export function filterImportedTeamSkills(
  ...
  // Not curated → the full configured catalog still appears (no upgrade break).
