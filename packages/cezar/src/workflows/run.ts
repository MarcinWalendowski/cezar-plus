...
import { isPidAlive, isSpoolLive, readSpoolMeta, spoolDirFor } from '../core/run-spool.ts';
...
export function reapAbandonedBroker(spoolDir: string): void {
  ...
  private async runContinuation(
    ...
    try {
      ...
      if (!retriedColdBroker && isRetryableBrokerLaunch(err)) {
        ...
        reapAbandonedBroker(err.spoolDir);
        this.store.appendEvent(runId, {
          type: 'note',
          stepId,
    ...
    while (i < workflow.steps.length) {
      ...
      if (kind === 'agent') {
        ...
        if (failure && coldBroker && !retriedColdBroker.has(step.id)) {
          ...
          reapAbandonedBroker(coldBroker.spoolDir);
          this.store.updateStep(runId, step.id, {
            sessionId: undefined,
            status: 'pending',
            error: undefined,
