...
describe('systemPrompt end-to-end (dry run)', () => {
  ...
  async function runToEnd(
    ...
    // Prompt capture owns the first mock call in these tests. Pin a model so the independent
    // unpinned-task classifier does not write its own system prompt into the same capture file.
    const record = manager.startRun(selectedWorkflow, { ...input, author: localCliAuthor(), model: 'sonnet' });
  ...
  it('live refresh: the namer applies turn context through the mock (direct drive)', async () => {
    const record = manager.startRun(skillWorkflow, { author: localCliAuthor(), model: 'sonnet', task: '437' });
...
describe('the global follow-up gate (dry run)', () => {
  ...
  async function runToEnd(input: { task: string; generateFollowups?: boolean }): Promise<string> {
    writeFileSync(argsFile, '', 'utf8');
    const record = manager.startRun(workflow, { ...input, author: localCliAuthor(), model: 'sonnet' });
    const terminal = new Set(['done', 'review', 'failed', 'cancelled']);
    const deadline = Date.now() + 20_000;
    while (!terminal.has(store.getRun(record.id)?.status ?? '')) {
      if (Date.now() > deadline) throw new Error('run did not finish in time');
