  ...
  resultingTasks: z
    .array(
      z.object({
        ...
        createdAt: z.string(),
        /**
         * Which leg of the pipeline this run is (`.ai/specs/2026-08-14-note-to-spec-pipeline.md`,
         * D3). Approving a proposal starts a **spec** run — an agent that reads the target repo's
         * knowledge, specs and git history and writes a spec with phases, then stops. The
         * **implementation** run is a separate, later click.
         *
         * Defaulted rather than required so a record written before this key existed reads back as
         * what it was: every run this feature had created at that point was a spec run.
         */
        kind: z.enum(['spec', 'implementation']).default('spec'),
        /** Repo-relative path of the spec the spec run reported writing. Absent until it has
         *  finished, and absent forever on a run that wrote none — which is a fact worth being able
         *  to see, not one to paper over with a guessed path. */
