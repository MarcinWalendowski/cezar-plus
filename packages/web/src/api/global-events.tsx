...
import { apiPath, getApiScope } from '@loki-labs/better-cezar-api-client'
 ...
 */
const WORKSPACE_EVENT_NAMES = ['project-added', 'project-removed', 'checkout-progress', 'automation-change', 'source-sync'] as const
    ...
    const connect = (): void => {
      ...
      for (const name of WORKSPACE_EVENT_NAMES) {
        source.addEventListener(name, (event) => {
          ...
          if (name === 'source-sync') {
            const project = payload && typeof payload === 'object' && typeof (payload as { project?: unknown }).project === 'string'
              ? (payload as { project: string }).project
              : undefined
            if (project) {
              void queryClient.invalidateQueries({
                predicate: (query) => query.queryKey[0] === project && query.queryKey[1] === 'sources',
              })
            }
          } else if (name !== 'checkout-progress' && name !== 'automation-change') {
