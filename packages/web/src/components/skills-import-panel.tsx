...
/**
 * The "Manage skills" panel: which of the team skills offered by a configured `skillsRepos` entry
 * reach the catalog and the composer picker. Every offered skill is enabled by default (opt-out)
 * and the user unchecks the ones they don't want. cezar ships no default repo
 * (2026-08-16-remove-open-mercato-coupling), so with none configured this list is simply empty and
 * the catalog is whatever is on the machine.
 *
 * The selection lives in the GLOBAL `~/.cezar/ui-state.json` (`importedSkills`, via the workspace
  ...
  return (
    <div data-slot="skills-import-panel" className="mx-auto w-full max-w-2xl">
      ...
      <p className="mt-2 text-[13px] leading-relaxed text-muted-foreground">
        Team skills from the repositories listed in <code>skillsRepos</code> (
        <code>.ai/cezar/config.json</code>). They&apos;re all in your catalog and the composer
        picker by default; uncheck any you don&apos;t want. Skills that live on this machine —{' '}
        <code>.ai/skills</code>, <code>~/.claude/skills</code> — are always available and are not
        listed here.
      </p>
      ...
      <div data-slot="import-list" className="mt-3 flex flex-col gap-1.5">
        {importable.isPending ? (
          ...
          <p className="px-1 py-2 text-xs text-soft-foreground">
            {/* Empty has TWO causes now that cezar ships no default repo, and the panel cannot tell
                them apart — `GET /skills/importable` answers `[]` for both. Naming only the clone
                would be a lie on a stock install, where nothing is configured to clone. */}
            {all.length > 0
              ? '(no skills match)'
              : '(no team skills — none configured in skillsRepos, or the repo is still cloning)'}
          </p>
        )}
      </div>
    </div>
