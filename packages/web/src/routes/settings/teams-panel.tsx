...
import { LockIcon, LogInIcon, TriangleAlertIcon } from 'lucide-react'
...
function TeamsBody({ probe }: { probe: TeamsProbe }) {
  if (probe.kind === 'auth-off') {
    return (
      <CenteredState
        icon={<LockIcon />}
        tone="neutral"
        title="Sign-in isn't set up on this deployment"
        subtitle="This cezar instance runs without authentication — there is no organization to manage teams for."
        heading="h2"
      />
    )
  }
  if (probe.kind === 'signed-out') {
    return (
      <CenteredState
        ...
        subtitle="This cezar instance requires sign-in. Continue with your identity provider to see and manage your organization's teams."
        actions={
...
function CreateTeamField() {
    ...
    create.mutate(
      { name: trimmedName, slug: slug.trim() },
      {
        onSuccess: () => {
          ...
          setName('')
          setSlug('')
          toast(`Team "${trimmedName}" created`)
        },
      },
    )
  }
  ...
  return (
    <SettingsField
      title="Create a team"
      hint="Teams group projects for filtering — engineering, marketing, whatever your org uses. Creating or renaming a team needs an owner or admin role; assigning a project to one happens from Settings → Projects."
    >
      ...
      {create.isError ? (
        <p data-slot="teams-create-error" role="alert" className="text-[13px] text-danger">
          {create.error instanceof Error ? create.error.message : 'could not create the team'}
        </p>
      ) : null}
    </SettingsField>
  )
...
function TeamsTable({ teams }: { teams: Team[] }) {
  ...
  return (
    <SettingsField
      title="Teams"
      hint="Every team in your organization."
    >
      {sorted.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-md border border-border">
          <table className="w-full border-collapse text-sm">
            <caption className="sr-only">Teams in your organization</caption>
            <thead>
